...
const quickSort_invert = [[], [], []];

for (const index in ten) {
  runCode(quickSort, ten[index], timeinvertTen[index]);
  runCode(quickSort, ten[index], timeinvertTen[index]);
  runCode(quickSort, ten[index], timeinvertTen[index]);
  quickSort_invert[0].push(calculaMedia(timeinvertTen[index]));
}
...
for (const index in twenty) {
  ...
  runCode(quickSort, twenty[index], timeinvertTwenty[index]);
  runCode(quickSort, twenty[index], timeinvertTwenty[index]);
  quickSort_invert[1].push(calculaMedia(timeinvertTwenty[index]));
}
...
for (const index in thirty) {
  ...
  runCode(quickSort, thirty[index], timeinvertThirty[index]);
  runCode(quickSort, thirty[index], timeinvertThirty[index]);
  quickSort_invert[2].push(calculaMedia(timeinvertThirty[index]));
}

console.log("cabei o invert do quick");

module.exports = quickSort_invert;
