...
const heapSort_invert = [[], [], []];

for (const index in ten) {
  runCode(heapSort, ten[index], timeinvertTen[index]);
  runCode(heapSort, ten[index], timeinvertTen[index]);
  runCode(heapSort, ten[index], timeinvertTen[index]);
  heapSort_invert[0].push(calculaMedia(timeinvertTen[index]));
}

for (const index in twenty) {
  runCode(heapSort, twenty[index], timeinvertTwenty[index]);
  runCode(heapSort, twenty[index], timeinvertTwenty[index]);
  runCode(heapSort, twenty[index], timeinvertTwenty[index]);
  heapSort_invert[1].push(calculaMedia(timeinvertTwenty[index]));
}

for (const index in thirty) {
  runCode(heapSort, thirty[index], timeinvertThirty[index]);
  runCode(heapSort, thirty[index], timeinvertThirty[index]);
  runCode(heapSort, thirty[index], timeinvertThirty[index]);
  heapSort_invert[2].push(calculaMedia(timeinvertThirty[index]));
}

console.log("cabei o invert do heap");

module.exports = heapSort_invert;
